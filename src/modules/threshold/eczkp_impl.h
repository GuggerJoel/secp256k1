...
int secp256k1_eczkp_pi_generate(const secp256k1_context *ctx, secp256k1_eczkp_pi *pi, const secp256k1_eczkp_parameter *zkp, const secp256k1_paillier_encrypted_message *m1, const secp256k1_paillier_encrypted_message *m2, const secp256k1_scalar *sx1, const secp256k1_scalar *sx2, const secp256k1_pubkey *c, const secp256k1_pubkey *w1, const secp256k1_pubkey *w2, const secp256k1_paillier_pubkey *pubkey, const secp256k1_eczkp_rdn_function rdnfp) {
    ...
    mpz_t n, n3, n3tild, nntild, tmp1, tmp2, tmp3, x1, x2;
    ...
    mpz_inits(alpha, beta, gamma, phi1, delta, mu, nu, phi2, phi3, epsilon, u2, u3,
        v3, v4, n, n3, n3tild, nntild, tmp1, tmp2, tmp3, 
        x1, x2, NULL);
...
int secp256k1_eczkp_pi2_generate(const secp256k1_context *ctx, secp256k1_eczkp_pi2 *pi2, const secp256k1_eczkp_parameter *zkp, const secp256k1_paillier_encrypted_message *m1, const secp256k1_paillier_encrypted_message *m2, const secp256k1_paillier_encrypted_message *m3, const secp256k1_paillier_encrypted_message *m4, const secp256k1_paillier_encrypted_message *r, const mpz_t x1, const mpz_t x2, const mpz_t x3, const mpz_t x4, const mpz_t x5, const secp256k1_pubkey *c, const secp256k1_pubkey *w2, const secp256k1_paillier_pubkey *pairedkey, const secp256k1_paillier_pubkey *pubkey, const secp256k1_eczkp_rdn_function rdnfp) {
    unsigned char ser65[65], b32[32], n32[32] = {
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 
        0xba, 0xae, 0xdc, 0xe6, 0xaf, 0x48, 0xa0, 0x3b, 0xbf, 0xd2, 0x5e, 0x8c, 0xd0, 0x36, 0x41, 0x41
    };
    ...
    mpz_t u2, u3, v3, v4, z3, v5;
    mpz_t n, n3, n5, n6, n7, n8, n3tild, n5tild, n7tild, nntild, tmp1, tmp2, tmp3;
    secp256k1_pubkey u1, v1, v2, pub1, pub2;
    secp256k1_sha256 hash;
    ...
    ARG_CHECK(rdnfp != NULL);
    secp256k1_sha256_initialize(&hash);
    mpz_inits(alpha, beta, gamma, phi1, delta, mu, nu, phi2, phi3, phi4, epsilon, sigma, tau, 
        u2, u3, v3, v4, z3, v5, n, n3, n5, n6, n7, n8, n3tild, n5tild, n7tild, nntild, 
        tmp1, tmp2, tmp3, NULL);
    ...
    mpz_mul(nntild, n, zkp->modulus);
    VERIFY_CHECK(mpz_cmp(pubkey->modulus, n6) > 0);
    VERIFY_CHECK(mpz_cmp(pairedkey->modulus, n8) > 0);
    /* Random values */
    rdnfp(alpha, n3, SECP256K1_THRESHOLD_RND_STD);
    rdnfp(beta, pubkey->modulus, SECP256K1_THRESHOLD_RND_INV);
    rdnfp(gamma, n3tild, SECP256K1_THRESHOLD_RND_STD);
    rdnfp(phi1, nntild, SECP256K1_THRESHOLD_RND_STD);
    rdnfp(delta, n3, SECP256K1_THRESHOLD_RND_STD);
    rdnfp(mu, pairedkey->modulus, SECP256K1_THRESHOLD_RND_INV);
    rdnfp(nu, n3tild, SECP256K1_THRESHOLD_RND_STD);
    rdnfp(phi2, nntild, SECP256K1_THRESHOLD_RND_STD);
    rdnfp(phi3, n, SECP256K1_THRESHOLD_RND_STD);
    ...
    VERIFY_CHECK(secp256k1_ec_pubkey_combine(ctx, &v2, pubs, 2) == 1);
    /* v3 */
    mpz_powm(tmp1, m3->message, alpha, pairedkey->bigModulus);
    mpz_powm(tmp2, m4->message, delta, pairedkey->bigModulus);
    mpz_mul(tmp3, tmp1, tmp2);
    mpz_mul(tmp1, n, sigma);
    mpz_powm(tmp2, pairedkey->generator, tmp1, pairedkey->bigModulus);
    mpz_mul(tmp1, tmp2, tmp3);
    mpz_powm(tmp2, mu, pairedkey->modulus, pairedkey->bigModulus);
    mpz_mul(tmp3, tmp1, tmp2);
    mpz_mod(v3, tmp3, pairedkey->bigModulus);
    /* v4 */
    mpz_powm(tmp1, zkp->h1, delta, zkp->modulus);
    mpz_powm(tmp2, zkp->h2, nu, zkp->modulus);
    mpz_mul(tmp3, tmp1, tmp2);
    ...
    mpz_powm(tmp1, m1->nonce, pi2->e, pubkey->modulus);
    mpz_mul(tmp2, tmp1, beta);
    mpz_mod(pi2->s2, tmp2, pubkey->modulus);
    mpz_mul(tmp1, pi2->e, phi1);
    mpz_add(pi2->s3, tmp1, gamma);
    mpz_mul(tmp1, pi2->e, x1);
    mpz_mul(tmp2, tmp1, x4);
    mpz_add(pi2->s4, tmp2, alpha);
    mpz_mul(tmp1, pi2->e, x2);
    mpz_add(pi2->t1, tmp1, delta);
    mpz_mul(tmp1, pi2->e, phi3);
    mpz_add(tmp2, tmp1, epsilon);
    mpz_mod(pi2->t2, tmp2, n);
    mpz_powm(tmp1, r->nonce, pi2->e, pairedkey->modulus);
    mpz_mul(tmp2, tmp1, mu);
    mpz_mod(pi2->t3, tmp2, pairedkey->modulus);
    ...
    mpz_clears(alpha, beta, gamma, phi1, delta, mu, nu, phi2, phi3, phi4, epsilon, sigma, tau, 
        u2, u3, v3, v4, z3, v5, n, n3, n5, n6, n7, n8, n3tild, n5tild, n7tild, nntild, 
        tmp1, tmp2, tmp3, NULL);
    memset(b32, 0, 32);
    memset(ser65, 0, 65);
